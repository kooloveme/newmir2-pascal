...
(*  LHAExpand starts expanding at InStr's current position and continues to
    ...
    you'll need to set InStr.Position := 0 before calling.
*)
procedure CompBufferL(const InData: Pointer; InSize: LongInt; out OutData:
  Pointer; out OutSize: LongInt);
procedure UnCompBufferL(const InData: Pointer; InSize: LongInt; out OutData:
  Pointer; out OutSize: LongInt);
...
procedure MakeTable(nchar: TwoByteInt; BitLen: PByte; TableBits: TwoByteInt;
  Table: PWord);
var
...
begin
  ...
  while (j <= HeapSize) do
  begin
    if (j < HeapSize) and (Freq^[Heap^[j]] > Freq^[Heap^[SUCC(j)]]) then
      INC(j);
    if Freq^[k] <= Freq^[Heap^[j]] then
      Break;
    Heap^[I] := Heap^[j];
    I := j;
    j := I shl 1;
...
function MakeTree(NParm: TwoByteInt; Freqparm: PWord; LenParm: PByte; Codeparm:
  PWord): TwoByteInt;
var
...
var
  I: Word;
begin
  for I := DICSIZ to (DICSIZ + UCHARMAX) do
  begin
    Level^[I] := 1;
{$IFDEF PERCOLATE}
    Position^[I] := NUL;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      Output(LastMatchLen + (UCHARMAX + 1 - THRESHOLD), (Pos - LastMatchPos - 2)
        and Pred(DICSIZ));
      Dec(LastMatchLen);
