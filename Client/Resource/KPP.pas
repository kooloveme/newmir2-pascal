...
type
  ...
  public
    Key: string;
    FileName: string;
    constructor Create(sFile: string; CheckFreeTick: integer = 5000;
      FreeTextureTick: integer = 10000; sKey: string = '123456');
    ...
    procedure Insert(idx: integer; PNG: Pointer; Size: integer;
      Coor: TCoordinate); overload;
    ...
    property Coordinate[idx: integer]: TCoordinate read GetCoordinate
      write SetCoordinate;
...
begin
  if not FInited then
    Exit; // 如果文件没有初始化。则不干
  if TThread.GetTickCount - FLastCheckTick < FCheckFreeTick then
    Exit; // 上次时间和本次执行 间隔太小。则不干
  ...
  begin
    ...
    if not Assigned(Tex) then
      Continue;
    if TThread.GetTickCount - Tex.CheckTime > FFreeTextureTick then
    begin
      Tex.Free;
      List[I] := nil;
    end;
...
constructor TKPPFile.Create(sFile: string; CheckFreeTick: integer = 5000;
  FreeTextureTick: integer = 10000; sKey: string = '123456');
begin
  inherited Create;
  FileName := sFile;
...
begin
  Result := nil;
  if not((index >= 0) and (index < FCount)) then
    Exit;
...
begin
  ...
  Result.CheckTime := TThread.GetTickCount;
...
begin
  ...
  if FCount > 0 then
    SetLength(FIdxDescList, FCount);
  if FHeader.IdxDescLength <> FileStream.
    Read(FIdxDescList[0], FCount * SizeOf(TKPPIdxDesc)) then
    Exit;
