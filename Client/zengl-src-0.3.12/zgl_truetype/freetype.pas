...
const
  ...
  function TT_Get_Glyph_Bitmap( _glyph   : TT_Glyph;
                                ...
                                y_offset : TT_F26Dot6; 
                                italic : boolean;
                                rasterizer: TFreeTypeCustomRasterizer = nil) : TT_Error;
  ...
  (*                                                               *)
  function TT_Translate_Outline( var aout : TT_Outline;
                                 x, y    : TT_F26Dot6 ) : TT_Error;

  (*****************************************************************)
  (*  Apply a 2x2 transform to an outline                          *)
  (*                                                               *)
  function TT_Transform_Outline( var aout : TT_Outline;
  ...
  (*                                                               *)
  function TT_Get_Outline_Bitmap( var aout : TT_Outline;
                                  var map : TT_raster_Map; rasterizer: TFreeTypeCustomRasterizer = nil  ) : TT_Error;

  (*****************************************************************)
  (*  Render an outline into a pixmap                              *)
  (*                                                               *)
  function TT_Get_Outline_Pixmap( var aout : TT_Outline;
                                  var map : TT_raster_Map; rasterizer: TFreeTypeCustomRasterizer = nil  ) : TT_Error;

  function TT_Get_Outline_Pixmap_HQ( var aout : TT_Outline;
                                  var map : TT_raster_Map; rasterizer: TFreeTypeCustomRasterizer = nil  ) : TT_Error;

  function TT_Render_Directly_Outline_Gray( var aout : TT_Outline;
                                  x, y, tx, ty: integer;
                                  OnRender: TDirectRenderingFunction; rasterizer: TFreeTypeCustomRasterizer = nil ) : TT_Error;

  function TT_Render_Directly_Outline_HQ( var aout : TT_Outline;
                                  x, y, tx, ty: integer;
                                  OnRender: TDirectRenderingFunction; rasterizer: TFreeTypeCustomRasterizer = nil ) : TT_Error;
  ...
  (*                                                               *)
  function TT_Get_Outline_BBox( var aout  : TT_Outline;
  ...
  (*                                                               *)
  function TT_Done_Outline( var aout : TT_Outline ) : TT_Error;
  ...
  function  TT_Get_Name_String( face      : TT_Face;
                                nameIndex : integer;
                                var str   : Pointer;
                                var len   : integer ) : TT_Error;overload;

  function  TT_Get_Name_String( face      : TT_Face;
                                nameIndex : integer ) : string;overload;
...
uses
    ...
    if glyph <> nil then
      begin
        ...
        if italic then begin
         shear.xx := 1 shl 16;
         shear.xy := round((FItalicShear * ( 1 shl 16 ) ));
         shear.yx := 0;
         shear.yy := 1 shl 16;
  ...
  label
    Fail;
  begin
    aout.n_points   := n_points;
    aout.n_contours := n_contours;
    aout.points     := nil;
    aout.flags      := nil;
    aout.conEnds    := nil;
    aout.owner      := true;

    if Alloc( Pointer(aout.points), 2*n_points*sizeof(TT_Pos) ) or
       Alloc( Pointer(aout.flags),    n_points*sizeof(Byte)   ) or
       Alloc( Pointer(aout.conEnds),  n_contours*sizeof(Short) ) then
      goto Fail;
  ...
  begin
    if aout.owner then
      begin
        Free( Pointer(aout.conEnds) );
        Free( Pointer(aout.flags) );
        Free( Pointer(aout.points) );
        aout.n_points   := 0;
        aout.n_contours := 0;
  ...
  begin
    if rasterizer = nil then rasterizer := TTGetDefaultRasterizer;
    if rasterizer.Render_Glyph( aout, map ) then
  ...
  begin
    if rasterizer = nil then rasterizer := TTGetDefaultRasterizer;
    if rasterizer.Render_Gray_Glyph( aout, map, nil ) then
  ...
  begin
    if rasterizer = nil then rasterizer := TTGetDefaultRasterizer;
    if rasterizer.Render_Gray_Glyph_HQ( aout, map ) then
      TT_Get_Outline_Pixmap_HQ := error
    else
      TT_Get_Outline_Pixmap_HQ := TT_Err_Ok;
  end;

  function TT_Render_Directly_Outline_Gray(var aout: TT_Outline; x, y, tx,
    ty: integer; OnRender: TDirectRenderingFunction;
    rasterizer: TFreeTypeCustomRasterizer): TT_Error;
  begin
    if rasterizer = nil then rasterizer := TTGetDefaultRasterizer;
    if rasterizer.Render_Directly_Gray_Glyph( aout, x,y,tx,ty, OnRender, nil ) then
      TT_Render_Directly_Outline_Gray := error
    else
      TT_Render_Directly_Outline_Gray := TT_Err_Ok;
  end;

  function TT_Render_Directly_Outline_HQ(var aout: TT_Outline; x, y, tx,
    ty: integer; OnRender: TDirectRenderingFunction;
    rasterizer: TFreeTypeCustomRasterizer): TT_Error;
  begin
    if rasterizer = nil then rasterizer := TTGetDefaultRasterizer;
    if rasterizer.Render_Directly_Gray_Glyph_HQ( aout, x,y,tx,ty, OnRender ) then
  ...
  begin
    if x <> 0 then
      for n := 0 to aout.n_points-1 do
        inc( aout.points^[n].x, x );

    if y <> 0 then
      for n := 0 to aout.n_points-1 do
        inc( aout.points^[n].y, y );
