...
begin
  // RU: Укажем свою текстуру ?слой для спрайт? заодно устано?тся стандартны?параметр?врод?ширины ?высоты на основе данных ?кадр??текстуре.
...
begin
  // RU: ?? по сути эт?процедур?об?влен?только для пример? то вызове?основной мето?OnDraw класса zglCSprite2D.
...
begin
  ...
  // RU: Если спрайт выходи?за предел?по X, сраз?же удаляем ег?
  // EN: Delete the sprite if it goes beyond X.
  if X < -128 Then sengine2d.DelSprite( ID );

  // RU: Если спрайт выходи?за предел?по Y, ставим ег??очеред?на удаление
...
begin
  for i := 1 to 100 do
    begin
      // RU: Запрашивае??спрайтовог?менеджер?ново?"мест? по?спрайт :)
      // EN: Request a "place" for a new sprite :)
      ID := sengine2d.AddSprite();
      // RU: Создае?экземп??спрайт?CMiku. Аргументам?конструктора являют? са?менеджер ?будущи?ID для спрайт?
...
// RU: Удалит?100 спрайтов.
// EN: Delete 100 sprites.
procedure DelMiku;
  var
    i : Integer;
begin
  // RU: Удалим 100 спрайтов со случайны?ID.
...
begin
  ...
  // RU: Создае?экземп??zglCSEngine2D.
...
begin
  batch2d_Begin();
  // RU: Рисуем вс?спрайт?нахо?щиеся ?текуще?спрайтовом менеджер?
...
begin
  ...
  // RU: Выполняем обработк?всех спрайтов ?текуще?спрайтовом менеджер?
  // EN: Process all sprites contained in current sprite engine.
  sengine2d.Proc();

  // RU: По нажати?пробел?очистить вс?спрайт?
...
procedure Quit;
begin
  // RU: Очищае?па?ть от созданны?спрайтов.
